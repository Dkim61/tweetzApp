...
class TweetTestCase(TestCase):
    ...
    def test_tweet_list(self):
        ...
        response = client.get("/api/tweetz/")
    ...
    def test_action_like(self):
        client = self.get_client()
        response = client.post("/api/tweetz/action/", 
            {"id": 1, "action": "like"})
    ...
    def test_tweet_create_api_view(self):
        ...
        client = self.get_client()
        response = client.post("/api/tweetz/create/", request_data)
        self.assertEqual(response.status_code, 201)
    ...
    def test_tweet_detail_api_view(self):
        client = self.get_client()
        response = client.get("/api/tweetz/1/")
        self.assertEqual(response.status_code, 200)
        data = response.json()
        _id = data.get("id")
    ...
    def test_tweet_delete_api_view(self):
        client = self.get_client()
        response = client.delete("/api/tweetz/1/delete/")
        self.assertEqual(response.status_code, 200)
        client = self.get_client()
        response = client.delete("/api/tweetz/1/delete/")
        self.assertEqual(response.status_code, 404)
        response_incorrect_owner = client.delete("/api/tweetz/3/delete/")
